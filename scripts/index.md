...
- `scripts/run_tests.sh`: 通常テストを外向き通信遮断下で実行します。実疎通は `--live-smoke <profile>` で明示します。
- `scripts/run_live_smoke.py`: 固定profileのallowlistにあるcredentialだけを専用containerへ渡して実疎通テストを実行します。

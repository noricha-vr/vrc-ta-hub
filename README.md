...
2. **`.env.local` の作成:**
   ...
   `docker-compose.yaml` は `.env.local` を読み込むので、ここを編集して必要な環境変数を設定します。
   Google Calendar API / Gemini API / Discord OAuth の認証情報取得手順は [docs/setup.md](docs/setup.md) を参照してください。

   ローカル起動時に最低限そろえておきたい環境変数:
   - `SECRET_KEY`
   ...
   機能によって追加で必要になる環境変数:
   - `OPENROUTER_API_KEY`: 記事自動生成や一部の LLM 機能で利用
   - `GEMINI_MODEL`: 省略時は `google/gemini-2.5-flash-lite-preview-06-17`
   - `GOOGLE_CALENDAR_CREDENTIALS`: 省略時は `/app/credentials.json`
   - `DISCORD_CLIENT_ID`, `DISCORD_CLIENT_SECRET`: Discord ログインを使う場合に必要

...
class AddStaffViewTest(TestCase):
    ...
    def test_owner_can_add_staff(self):
        ...
        response = self.client.post(
            reverse('community:add_staff', kwargs={'pk': self.community.pk}),
            {'user_name': '新規ユーザー'}
        )
    ...
    def test_add_nonexistent_user(self):
        ...
        # メンバー数が増えていないことを確認
        self.assertEqual(
            CommunityMember.objects.filter(community=self.community).count(),
            1  # オーナーのみ
        )

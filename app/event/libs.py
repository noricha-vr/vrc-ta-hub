...
def generate_blog(event_detail: EventDetail, model='google/gemini-2.0-flash-001') -> BlogOutput:
    ...
    try:
        ...
        # PDFの内容とURLを取得
        pdf_content = ""
        pdf_url = event_detail.slide_url or (
            event_detail.slide_file.url if event_detail.slide_file else "")
        ...
        if event_detail.slide_file:
            ...
            try:
                ...
                logger.warning(
                    f"Error loading PDF for EventDetail {event_detail.pk}: {e}")
            finally:
        ...
        prompt_text = BLOG_GENERATION_TEMPLATE.format(
            ...
            date=event_detail.event.date.strftime('%Y年%m月%d日') if hasattr(
                # 日付フォーマット（文字列の場合はそのまま）
                event_detail.event.date, 'strftime') else event_detail.event.date,
...
```"""
        ...
        try:
            ...
            logger.error(
                f"Failed to decode JSON response from OpenRouter: {e}\nAttempted JSON string: {json_str}")
            # JSONパース失敗時は空を返す
            return BlogOutput(title='', meta_description='', text='')
        except Exception as e:  # Pydanticのバリデーションエラーなども捕捉
            logger.error(
                f"Failed to create BlogOutput from parsed JSON: {e}\nParsed data attempt: {json_str}")
            return BlogOutput(title='', meta_description='', text='')

    except Exception as e:
        # APIキーエラーなどもここで捕捉される可能性がある
        logger.error(
            f"Error calling OpenRouter or processing response for EventDetail {event_detail.pk}: {e}")
        if "API key" in str(e):
            logger.error(
                "OPENROUTER_API_KEY environment variable might be missing or invalid.")
        return BlogOutput(title='', meta_description='', text='')
...
def generate_meta_description(text: str, model='google/gemini-2.0-flash-001') -> str:
    ...
    try:
        ...
        if len(meta_description) > 250:
            logger.warning(
                f"Generated meta description exceeded 250 characters ({len(meta_description)}). Trimming.")

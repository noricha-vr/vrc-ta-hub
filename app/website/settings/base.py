...
# Secret Manager に placeholder 値 (例: "placeholder-update-later") を一時的に登録した
# 期間でも起動失敗しないよう、URL scheme (http/https) で始まる値のみ Sentry 初期化対象とする。
# 不正な DSN だと sentry_sdk.init が起動時に BadDsn を投げてアプリ全体が落ちる。
_sentry_dsn_is_valid = SENTRY_DSN.startswith(('http://', 'https://'))
if _sentry_dsn_is_valid and not TESTING and not DEBUG:
    import sentry_sdk

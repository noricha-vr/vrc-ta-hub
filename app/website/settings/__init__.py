...
- base: Django コア（INSTALLED_APPS / MIDDLEWARE / DATABASES / CACHES など）
- logging_config: structlog 初期化 / LOGGING
...
順序は base → logging_config → security → apis → storage → caching → authentication
で固定。DEBUG / BASE_DIR などは base で定義され、後続モジュールは
`from .base import ...` で取り込む。
"""

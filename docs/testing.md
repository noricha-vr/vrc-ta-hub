...
`.github/workflows/ci.yml` は、head branch が `fix-flow/isolation-task-*` のPRについて、
trusted reviewer が `safe-to-test` label を付けるまでPR内のコードを実行しない。
`isolation-gate` job自体はcheckoutやPR内actionを使わず、それ以外の全jobがこのgateに
直接依存する。通常branchのPRと`main`へのpushは従来どおり実行する。

承認手順:

1. reviewerがartifact差分とPRのexact head commitを確認する
2. 対象branchがGitHub rulesetで更新禁止され、承認後にSHAを差し替えられないことを確認する
3. repositoryのlabel操作権限を持つtrusted reviewerが`safe-to-test`を付ける
4. `labeled` eventで再開したlint / test / e2e / mypyの結果を確認する

`safe-to-test` はPR labelでありcommit SHAに紐づかない。そのためbranchの不変化が
必須で、rulesetを設定する前に隔離publisherを有効化してはいけない。再修正が必要な場合は
既存branchを更新せず、新しいartifact / branch / PRを作って改めて承認する。
`safe-to-test`を外すと`unlabeled` eventが実行中のPR workflowをキャンセルし、
新しいgateが失敗する。

workflowへjobを追加する場合は、job-levelの`if` でlabel eventをskipせず、
`needs` に`isolation-gate`を直接含める。この構造は
`website.tests.test_ci_workflow` で回帰検証する。

...
> **ステータス**: ✅ 対応完了（履歴ドキュメント）。`STORAGES` への移行は実装済み。
> 設定ファイルは PR #415 で `app/website/settings/` ディレクトリに分割され、
> storage 関連は `app/website/settings/storage.py` に集約されている。

## 概要

Issue #264 の受け入れ条件に沿って、Django 4.2 環境で `-Wa` を付けた warning 表示を確認した。

結論として、プロジェクト由来の Django deprecation warning は当時の `app/website/settings.py` の storage 設定から発生していた（現在は `app/website/settings/storage.py` に分割済み）。当該 PR では保護対象ファイルだったため設定変更を行わなかったが、後続作業で `STORAGES` への移行は完了している。
...
| warning | 当時の発生元 | 対応 |
| --- | --- | --- |
| `DEFAULT_FILE_STORAGE setting is deprecated` | `app/website/settings.py` (現在は `settings/storage.py`) | ✅ 完了。`STORAGES["default"]` へ移行済み。 |
| `STATICFILES_STORAGE setting is deprecated` | `app/website/settings.py` (現在は `settings/storage.py`) | ✅ 完了。`STORAGES["staticfiles"]` へ移行済み。 |
...
1. ~~`app/website/settings.py` の `DEFAULT_FILE_STORAGE` / `STATICFILES_STORAGE` を `STORAGES` に移行する。~~ ✅ 完了 (`app/website/settings/storage.py` に集約)

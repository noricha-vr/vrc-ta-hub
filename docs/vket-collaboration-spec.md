...
6. 公開イベントを直接変更した可能性がある日時差分の警告が出た場合、自動修正はしない。意図する日時を確認し、確定日程を合わせるか、公開同期を再実行して公開イベントを確定日程へ戻す。`published_event` が未設定でも、ACTIVE な参加と同じ集会かつ確定日程・確定開始時刻に一致する `Event` がない場合は、日時ロックが機能していない警告を表示する。確定日程または確定開始時刻が未設定の参加は、この警告の対象外である。

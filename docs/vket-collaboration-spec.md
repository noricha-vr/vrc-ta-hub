...
4. 完了メッセージの件数と各参加の進捗が「完了」であることを確認する。日程・開催時間差分の警告がないことは、日付・開始時刻・開催時間の同期確認に使う。
5. 公開同期は再実行できる。確定日程・確定開始時刻・確定開催時間を運営画面で変更した場合は、必ず再実行して公開イベントへ反映する。
6. 公開イベントを直接変更した可能性がある日程・開催時間差分の警告が出た場合、自動修正はしない。意図する日程・開催時間を確認し、確定内容を合わせるか、公開同期を再実行して公開イベントを確定内容へ戻す。`published_event` が未設定でも、ACTIVE な参加と同じ集会かつ確定日程・確定開始時刻に一致する `Event` がない場合は、日時ロックが機能していない警告を表示する。確定日程または確定開始時刻が未設定の参加は、この警告の対象外である。
